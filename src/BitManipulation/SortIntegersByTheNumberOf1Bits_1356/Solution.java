...
/**
 ...
 Given an integer array arr. You have to sort the integers in the array in ascending order by the
   number of 1's in their binary representation and in case of two or more integers have the
...
public class Solution {
    public int[] sortByBits(int[] arr) {
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            list = tree.getOrDefault(bitsCount, new ArrayList());
            list.add(arr[i]);

            tree.put(bitsCount, list);
        }

        for(Map.Entry e : tree.entrySet()) {
            list = tree.get(e.getKey());
