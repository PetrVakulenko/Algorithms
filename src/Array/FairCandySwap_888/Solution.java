...
/**
 ...
 Alice and Bob have candy bars of different sizes:
    ...
    B[j] is the size of the j-th bar of candy that Bob has.
...
public class Solution {
    public int[] fairCandySwap(int[] A, int[] B) {
        int difference = (sumCandies(A) - sumCandies(B)) / 2;
        ...
        for (int i = 0; i < A.length; i++) {
            if (s.contains(A[i] - difference)) {
                res[0] = A[i];
                res[1] = A[i] - difference;
            }
        }
